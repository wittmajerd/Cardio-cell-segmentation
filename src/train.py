...
from src.evaluate import evaluate, evaluate_all_metrics, predict_image
        ...
        with tqdm(total=len(train_loader.dataset), desc=f'Epoch {epoch}/{epochs}', unit='img') as pbar:
            for i, (batch) in enumerate(train_loader):
                ...
                if (i+1) == len(train_loader):
                    metrics = evaluate_all_metrics(model, val_loader, device, scale)
                    val_score = metrics['dice_score']
